...
type Props = { canvas?: HTMLCanvasElement; label: string; background: string; onSample?: (color: string) => void; onHover: (color: PixelReading | null) => void };
type Camera = { x: number; y: number; scale: number };
export default function CanvasPreview({ canvas, label, background, onSample, onHover }: Props) {
  ...
  const fit = () => canvas ? Math.max(.01, Math.min((size.width - 36) / canvas.width, (size.height - 36) / canvas.height)) : 1;
  const resetCamera = () => {
    onZoom(0);
  ...
  useEffect(() => {
    ...
    display.current.getContext("2d")!.drawImage(canvas, 0, 0);
  }, [canvas]);
  useLayoutEffect(() => {
    if (!canvas) return;
    ...
    setCamera(old => {
      ...
      return { scale, x: size.width / 2 - (size.width / 2 - old.x) * ratio, y: size.height / 2 - (size.height / 2 - old.y) * ratio };
    });
    setHover(null);
  ...
  function read(clientX: number, clientY: number) {
    ...
    const x = Math.floor((clientX - rect.left - camera.x) / camera.scale);
    const y = Math.floor((clientY - rect.top - camera.y) / camera.scale);
