...
import { ArrowDownToLine, ArrowLeftRight, BookmarkPlus, Check, CheckCheck, ChevronRight, Copy, Crop, Download, FileArchive, FileImage, FlipHorizontal2, FolderOpen, ImagePlus, Layers3, LoaderCircle, Moon, Sun, Palette, Pipette, Plus, Redo2, RotateCw, ScanLine, ShieldCheck, Undo2, Upload, X } from "lucide-react";
...
export default function Studio() {
  ...
  const sampleRef = useRef<SVGSVGElement>(null);
  ...
  async function example() {
    ...
    try {
      const svg = new XMLSerializer().serializeToString(sampleRef.current);
      const asset = await importAsset(new File([svg], "icone-exemplo.svg", { type: "image/svg+xml" }));
      asset.example = true;
      const png = await canvasBlob(asset.canvas);
      asset.name = "icone-exemplo.png"; asset.size = png.size;
      setAssets(old => [...old, asset]); setActiveId(asset.id);
  ...
  const sample = (color: string) => { if (picking) { patch({ [picking]: color }); setPicking(null); toast.success(`Cor capturada: ${color.toUpperCase()}`); } };
  ...
  return <TooltipProvider delayDuration={250}><div className="studio" onDragOver={e => { if (e.dataTransfer.types.includes("Files")) { e.preventDefault(); setDrag(true); } }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDrag(false); }} onDrop={e => { e.preventDefault(); setDrag(false); void importFiles([...e.dataTransfer.files]); }}>
    ...
    <SidebarProvider className="editor-layout" style={{ "--sidebar-width": "204px" } as CSSProperties}>
      ...
      <main className="editor-main"><div className="workspace-heading"><div><div className="breadcrumb">Editor de ícones <ChevronRight size={12} /> {navigation.find(n => n.id === tool)?.title}</div><h1>{navigation.find(n => n.id === tool)?.title}</h1></div><div className="history-controls"><IconButton label="Desfazer" onClick={() => history()} disabled={!active?.past.length}><Undo2 size={18} /></IconButton><IconButton label="Refazer" onClick={() => history(true)} disabled={!active?.future.length}><Redo2 size={18} /></IconButton><span className="toolbar-separator" /><IconButton label="Restaurar original" onClick={reset} disabled={!active}><RotateCw size={17} /></IconButton></div></div>
        ...
        <div className={`previews ${view === "result" ? "result-only" : ""}`}>
          {view === "split" && <section className="preview-panel"><header><span><span className="preview-dot" />Original</span><div>{active && <small>{active.width} × {active.height} px</small>}<IconButton label="Copiar original" disabled={!active} onClick={() => copyResult(true)}><Copy size={15} /></IconButton><IconButton label="Baixar original" disabled={!active} onClick={() => active && canvasBlob(active.canvas).then(blob => download(blob, `${safeName(active.name)}-original.png`)).catch(() => toast.error("Falha ao baixar original."))}><ArrowDownToLine size={15} /></IconButton></div></header>
            {active ? <Preview key={`${active.id}-original`} canvas={active.canvas} label="Imagem original" background={background} onHover={setLivePixel} onSample={picking ? sample : undefined} /> : <div className="import-area checker"><div className="import-symbol"><ImagePlus size={32} strokeWidth={1.25} /></div><h2>Seu próximo ícone começa aqui</h2><button className="button primary" onClick={() => fileInput.current?.click()}><Plus size={16} />Abrir arquivos</button><span className="file-formats">PNG, JPG, WebP, SVG e ICO</span><button className="text-action example-action" onClick={example}>Abrir ícone de exemplo <ChevronRight size={14} /></button></div>}
          ...
          <section className="preview-panel output-panel"><header><span><span className="preview-dot result-dot" />{mask ? "Máscara" : "Resultado"}</span><div>{active && <small>{!ready && !renderError ? <LoaderCircle size={14} className="spin" /> : output ? `${mask ? active.width : output.canvas.width} × ${mask ? active.height : output.canvas.height} px` : ""}</small>}{mask && <IconButton label="Baixar máscara de alterações" disabled={!output} onClick={() => output && canvasBlob(output.mask).then(blob => download(blob, "mascara-de-alteracoes.png")).catch(() => toast.error("Falha ao baixar máscara."))}><Download size={15} /></IconButton>}<IconButton label="Copiar resultado" onClick={() => copyResult()} disabled={!active}><Copy size={15} /></IconButton></div></header>
            {renderError ? <div className="render-error"><ScanLine size={28} /><p>{renderError}</p></div> : <Preview key={`${active?.id ?? "empty"}-result`} canvas={output ? mask ? output.mask : output.canvas : undefined} label={mask ? "Máscara: preto indica pixels alterados; branco, pixels preservados" : "Imagem resultante"} background={background} onHover={setLivePixel} onSample={picking && output && !mask ? sample : undefined} />}
      ...
      <aside className="inspector"><div className="live-pixel-panel"><div className="live-pixel-heading"><Pipette size={15} /><span>Cor sob o cursor</span></div><div className="live-pixel-value"><span className="pixel-swatch checker"><span style={{ background: livePixel?.rgba ?? "transparent" }} /></span><div><strong>{livePixel ? livePixel.alpha ? livePixel.hex : "Transparente" : "—"}</strong><small>{livePixel ? livePixel.rgba : "Nenhum pixel selecionado"}</small></div></div>{livePixel && <div className="pixel-position">{livePixel.label} · X {livePixel.x} · Y {livePixel.y}</div>}</div><Adjustments asset={active} settings={s} tool={tool} patch={patch} picking={picking} pick={field => { setPicking(picking === field ? null : field); setView("split"); }} mask={mask} setMask={setMask} reset={reset} saved={saved} saveColor={saveColor} removeColor={removeColor} setSavedColors={setSavedColors} /><div className="inspector-footer"><CheckCheck size={16} /><span>Original preservado</span></div></aside>
    </SidebarProvider>
